...
echo $gid=(int)($club_file[(int)($num[0])]);
...
//проверяем юзверя на дату	
$date = date("d.m.y");	
include("../../config.php");
$result = mysql_query("select * from tabla2 WHERE uid='$user' ORDER BY id DESC");
...
if($check_data=="" or $check_data<>$date ) {
...
mysql_query("DELETE FROM tabla2 WHERE uid = '$uid'");
...
values ('$uid','$data', '$time', '$club_id', '$first_name',  '$second_name', '$sex', '$country' , '$city', '$country_id[1]' , '$city_id[1]', '$friends', '$followers', '$avatar')" );
}
