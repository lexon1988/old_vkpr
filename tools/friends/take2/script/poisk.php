<?php
include('../../../../header/header_takefriends_poisk2.php');
?>
...
//echo "Позовчера: ".$yesterday2;
 ...
 $result = mysql_query("select * from tabla2 WHERE uid>0 $sql_check $sql_sex $sql_country $sql_city $sql_country_id $sql_city_id $sql_friends1 $sql_friends2 $sql_followers1 $sql_followers2 AND (data='$date' OR data='$yesterday' OR data='$yesterday2') ORDER BY id DESC limit $start_pos, $perpage");
...
<td  valign='center' width="60px" rowspan='2'style='border:1px solid #DEE0EB; border-left :0px;  border-top:0px;text-align: center;' ><div class="div1" ><font color="white" class="ws48"><B><a href='http://vk.com/id<?php echo $row["uid"];?>' target='_blank' class='plus' rel='nofollow'>+</a></B></font></div></td>
...
<td style='border:1px solid #DEE0EB; text-align: center; border-right :0px; border-left :0px; border-top:0px;'> <?php echo "<div class='anoncetxt'><img src='".$row["avatar"]."'  width= 50px height= 50px></div>"; ?><font size=2 color=black> <?php echo $row["first_name"]." ".$row["second_name"] ; ?></font></td>
